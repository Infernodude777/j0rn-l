-- =====================================================
-- J0RN@L — Consolidated init (paste into Supabase SQL editor)
-- =====================================================
-- Creates: profiles, daily_checkins, journal_entries, weekly_insights,
...
-- Plus:    storage bucket `journal`, RLS policies on every table, trigger
--          that auto-creates profile + settings rows on signup.
--
-- Safe to run on a fresh project OR on an already-migrated one (all DDL
...
-- Free-text responses + LLM interpretation (was in 0003)
...
-- If a project already ran the old 0001_initial_schema.sql, the table exists
-- but is missing source / external_id / external_meta (those came from 0002).
-- Add them now, idempotently.
...
-- CONNECTED ACCOUNTS ==================================
-- provider check widened: must include `discord` (the bot writes it on
-- /link redemption) and every id in lib/providers/catalog.ts. If you add
-- a new provider to the catalog, append it here too.
...
-- If the table already existed from the old 0001 migration, its CHECK
-- constraint is the narrow 6-id version. Drop and re-add so the new
-- provider ids (discord, instagram, spotify, notion, todoist,
-- google_calendar, apple_calendar, screen_time) are accepted on insert.
-- IF EXISTS on the DROP handles the "fresh project" case; the DO-block
-- on the ADD swallows "constraint already exists" (Postgres has no
...
-- ============================================================
-- 2. Connector / messaging tables
-- ============================================================

-- LINKED ACCOUNTS (Discord, Instagram, browser extension, etc.) ===
...
-- SOCIAL USAGE DAILY (rollup cache) =====================
...
-- SHARE TOKENS (short-lived tokens for linking flows) ===
...
-- Drop every existing policy so the file is idempotent (you can re-run it
...
-- "self" policies: each user can CRUD their own rows.
...
-- ============================================================
-- 4. Trigger: auto-create profile + settings on signup
-- ============================================================
...
-- ============================================================
-- That's it. The /api/discord/cron endpoint runs under the service role
-- (CRON_SECRET-gated) so it bypasses RLS automatically — no extra policy
-- needed here.
-- ============================================================
